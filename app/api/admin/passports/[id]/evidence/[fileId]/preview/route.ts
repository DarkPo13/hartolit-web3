...
export async function GET(request: NextRequest, context: Context) {
  ...
  try { const { url } = await previewReviewEvidence(id, fileId); return NextResponse.redirect(url, { headers: { "Cache-Control": "private, no-store", "Referrer-Policy": "no-referrer" } }); }
